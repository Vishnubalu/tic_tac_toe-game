...
def calculateSystemChances():
    ...
    if len(obj.table_values) == len(obj.system_entries) + len(obj.player_entries):
        print(" ************ DRAW MATCH ************")
        printout.inspect(obj.system_entries, obj.player_entries)
    else:
        ...
        for value in chances_for_system:
            if value in obj.table_values and \
                    value not in obj.system_entries and \
                    value not in obj.player_entries:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if input("who place first s/u: ").lower() == 'u':
            printout = scrutiny.Scrutiny(obj.dis, obj.table_values, 0)
            entry_of_player()
        else:
            printout = scrutiny.Scrutiny(obj.dis, obj.table_values, 1)
            calculateSystemChances()
